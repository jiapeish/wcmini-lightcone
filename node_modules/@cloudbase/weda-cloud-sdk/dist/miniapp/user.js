...
const business_api_1 = require("../cloud-api/business-api");
const USER_INFO_CACHE_KEY = 'lcap-user-session';
const MAX_CACHE_DURATION = 3 * 24 * 60 * 60 * 1000;
// 缓存用户登录信息
const observedInfo = (0, mobx_1.observable)({ currentUser: null });
/**
 * 注册用户
...
async function signIn(params) {
    ...
    const { app } = await (0, tcb_1.getTcbInstance)();
    const wedaInfo = await (0, cloud_api_1.callCommonService)({
        // @ts-ignore
    ...
    if (!userInfo.wxOpenId) {
        try {
            const { Data: { Data }, } = await (0, cloud_api_1.callCommonService)({
...
async function getUrlWithOpenidToken(src) {
    if (wx === undefined || typeof wx.login !== 'function' || !src)
        return '';
    const { auth } = await (0, cloud_api_1.getCloudInstance)();
    if (!auth)
        return '';
    ...
    // 非全托管模式
    try {
        const tokenRes = await (0, business_api_1.callWedaApi)({
        ...
        const { provider_token: providerToken } = await auth.grantProviderToken({
            provider_id: miniProgram === null || miniProgram === void 0 ? void 0 : miniProgram.appId,
            provider_access_token: `${(0, common_1.getConfig)('envID')} ${token}`,
        });
        const url = generateUrl(providerToken);
        return url;
    }
...
exports.default = {
    ...
    unionIdLoginInWxApp,
    modifyCurrentUser,
    judgePasswordFreeLogin: tcb_1.judgePasswordFreeLogin,
};
