...
exports.createPage = exports.PAGE_ROOT_SYMBOL = void 0;
...
exports.PAGE_ROOT_SYMBOL = Symbol('@@page_route@@');
function resolveParticialPageData(ctx, key, data = {}, dataBinds = {}) {
    ...
    try {
        merge = (0, util_2.mergeDynamic2StaticData)(merge, pickedDataBinds, {
            codeContext: {
                /**
                 * $page 或 $comp 实例
                 */
    ...
    if (shareTimeline?.enable || lifecycle?.['onShareTimeline']) {
        result['onShareTimeline'] = () => {
            if (shareTimeline?.enable) {
                ...
                const query = (0, index_1.urlJoinParams)('/fake', Array.isArray(params)
                    ? params.reduce((map, { key, value }) => {
                        map[key] = value;
                        return map;
                    }, {})
...
function createPage({ app, id, widgetProps, lifecycle, state, computed, evtListeners, dataBinds = {}, handlers, query: datasetQuery = {}, eventFlows = [], pageContext = {}, pageAttributes = {}, resetShare = true, datasetProfile = undefined, checkAuth = async () => true, getExpiredMessage = async () => {
    return '';
}, style = {}, }) {
    /**
     * 单次引用
     */
    if (datasetProfile) {
        (0, weda_cloud_sdk_1.setDatasetProfiles)({ [pageContext.uuid || pageContext.id || id]: datasetProfile });
    }
    widgetProps = (0, util_2.patchWdigetPropsWithEvtListeners)(widgetProps, evtListeners);
    const evtHandlers = (0, util_1.createEventHandlers)(evtListeners);
    // @ts-ignore
    const { [exports.PAGE_ROOT_SYMBOL]: pageRootDataBinds, ...componentDataBinds } = dataBinds;
    const staticPageStyle = style;
    return Component({
        data: {
            ...(0, widget_1.createInitData)(widgetProps, componentDataBinds),
            _isCheckingAtuh: false,
            _expiredMessage: '',
            weDaHasLogin: null,
            loginError: null,
            pageStyle: (0, style_1.styleToCss)(staticPageStyle),
        },
        lifetimes: {
            attached() {
                this._disposers = [];
                    ...
                    pageStyle: () => {
                        ...
                        try {
                            if (pageRootDataBinds.style) {
                                const { style } = (0, util_2.mergeDynamic2StaticData)({}, {
                        ...
                        return (0, style_1.styleToCss)(pageStyle);
                    },
                }));
        ...
        methods: {
            ...
            async beforePageCustomLaunch(query) {
                if (!this._beforePageCustomLaunchPromise) {
                    this._beforePageCustomLaunchPromise = new Promise(async (resolve, reject) => {
                        ...
                        let weDaHasLogin;
                        if (query) {
                            weda_cloud_sdk_1.EXTRA_API.setParams($page.uuid, query);
                        }
                        try {
                            ...
                            if (auth) {
                                ...
                                (0, weda_cloud_sdk_1.createStateDataSourceVar)($page.uuid, (0, weda_cloud_sdk_1.generateParamsParser)({ app, $page, $w: $page.__internal__?.$w }));
                            }
                            else {
                                weDaHasLogin = false;
                                this.setData({
            ...
            async onLoad(options) {
                const $page = this._getInstance();
                (0, weda_cloud_sdk_1.setConfig)({ currentPageId: $page.uuid });
                this._pageActive = true;
                $page.__internal__.active = this._pageActive;
            ...
            _getInstance() {
                ...
                let dataset = (0, weda_cloud_sdk_1.createDataset)($page.uuid, undefined, {
                    appId: app.id,
                });
                dataset.query = (0, query_1.generateDatasetQuery)(datasetQuery, {
                    $w: $page.__internal__.$w,
                    $app: app,
                    $page,
                });
                ...
                this._disposers.push(...(0, watch_1.watchAndSyncDatasetState2Local)(app.id, $page.uuid, $page.dataset.state));
                ...
                const { widgets, rootWidget } = (0, widget_1.createWidgets)(widgetProps, componentDataBinds, this, $page.widgets);
                ...
                $page.__internal__.eventFlows = (0, flow_1.generateEventFlows)(eventFlows, {
                    $w: $page.__internal__.$w,
                    $app: app,
                    $page,
                });
