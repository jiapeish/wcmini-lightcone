...
exports.normalizeRouterParams = exports.parseURL = exports.getEnumValue = exports.SDK = exports.filterMap = exports.execOnce = exports.isSameArray = exports.upperFirst = exports.objectKeyFirstLowerToUpper = exports.lowerFirst = exports.objectKeyFirstUpperToLower = exports.createAPINamespace = exports.createAppUtils = exports.normalizePackageName = exports.normalizePageId = exports.urlJoinParams = exports.isMobile = exports.setter = exports.getter = void 0;
const vender_1 = require("../vender");
const date_1 = require("./date");
const Time = __importStar(require("./time/index"));
const Calc = __importStar(require("./calculation/index"));
...
*/
function getter(context, path, defaultValue = undefined) {
    return (0, vender_1.get)(context, path, defaultValue);
}
exports.getter = getter;
/*
设置 object对象中对应 path 属性路径上的值，如果path不存在，则创建。 缺少的索引属性会创建为数组，而缺少的属性会创建为对象。 使用_.setWith 定制path创建
*/
function setter(context, path, value = undefined) {
    return (0, vender_1.set)(context, path, value);
}
...
exports.urlJoinParams = urlJoinParams;
 ...
 * 创建应用 utils 实例
 */
function createAppUtils(ctx, initData) {
    let loading = {};
    // 留着占位 以后更新到这里
    if (!ctx.__internal__?.enumOptions) {
        (0, vender_1.set)(ctx, '__internal__.enumOptions', (0, mobx_1.observable)({}));
    }
    function getEnumOptions(optionName) {
        ...
        if (!loading[optionName]) {
            ...
            (0, index_1.getGeneralOptions)(optionName).then((data) => {
 ...
 * 创建应用 $w 代理
 */
function createAPINamespace(app, env, options) {
    const { device = {
        viewport: {
            width: 0,
            height: 0,
    ...
    const observableDevice = (0, mobx_1.observable)(device);
    const observableWedaContext = (0, mobx_1.observable)({
        isEditorMode: false,
        editorPlatforms: [],
    ...
    return new Proxy({
        ...
        get utils() {
            ...
            [...Object.values(constants_1.ACTIONS_KEY), ...Object.values(constants_1.ROUTER_KEY)]
                .filter((key) => key !== constants_1.ACTIONS_KEY.callProcess && key !== constants_1.ACTIONS_KEY.callWorkflow)
                .forEach((key) => {
...
exports.createAPINamespace = createAPINamespace;
/**
 * 将一个对象中每个key的首字母从大写变小写
 */
const objectKeyFirstUpperToLower = (oldObj) => {
    const newObj = {};
    if (oldObj && JSON.stringify(oldObj) !== '{}' && oldObj instanceof Object) {
        Object.keys(oldObj).forEach((item) => {
            newObj[(0, exports.lowerFirst)(item)] = oldObj[item];
        });
    }
    return newObj;
};
exports.objectKeyFirstUpperToLower = objectKeyFirstUpperToLower;
/**
 * 首字母转为小写
 * @param str
 * @returns
 ...
 */
const objectKeyFirstLowerToUpper = (oldObj) => {
    const newObj = {};
    if (oldObj && JSON.stringify(oldObj) !== '{}' && oldObj instanceof Object) {
        Object.keys(oldObj).forEach((item) => {
            newObj[(0, exports.upperFirst)(item)] = oldObj[item];
        });
    }
    return newObj;
};
exports.objectKeyFirstLowerToUpper = objectKeyFirstLowerToUpper;
/**
 * 首字母转为大写
 * @param str
 * @returns
 ...
 * 根据白名单过滤json
 */
function filterMap(whiteList, json) {
    const newJson = {};
 ...
 * @returns
 */
function getEnumValue({ enumOptions, optionSetName, key, } = {}, app) {
    try {
...
const normalizeRouterParams = (url) => {
    ...
    const routerParams = {
        params,
        url,
        protocol: constants_1.LINK_PROTOCOL[protocol],
    };
    const reg = /(?:([^\/]*))(?:\/([^?#]*))/;
...
exports.normalizeRouterParams = normalizeRouterParams;
