 ...
 */
import { getTcbInstance, CLOUD_SDK, getAccessToken as _getAccessToken, initTcb, } from '@cloudbase/weda-cloud-sdk';
import { callWedaApiFn } from '../../common/models';
import { objectKeyFirstUpperToLower, filterMap } from '../../common/utils';
import { getAdminAppInfo } from './permission';
...
export const userObj = {
     ...
     */
    get currentUser() {
        const res = filterCloudSdkUserInfo(CLOUD_SDK.currentUser);
        return res;
    },
};
/**
 ...
 * @returns
 */
export async function initWedaHostUser() {
    // 更新window._WedaHostConfig.wedaUser为cloud-sdk的信息，避免影响
    const [user, userAllInfo] = await Promise.all([CLOUD_SDK.getUserInfo(), getUserAllInfo()]);
 ...
 * @returns
 */
export function transformWedaHostUser(wedaUser, userAllInfo) {
    ...
    return {
        ...
        relatedRoles: relatedRoles.map((item) => {
            return objectKeyFirstUpperToLower(item);
        }),
 ...
 * @returns
 */
export async function onLoginStateChanged(cb) {
    const { auth } = await getTcbInstance();
    return auth.onLoginStateChanged(async (loginState) => {
        try {
            ...
            const [userInfo, appInfo] = await Promise.all([getUserInfo(), getAdminAppInfo({ uid: loginState?.user?.uid })]);
            const newAppInfo = appInfo.VisitApps.map((item) => objectKeyFirstUpperToLower(item));
            cb({
                userInfo,
                appInfo: newAppInfo,
            });
...
export async function getUserAllInfo() {
    return callWedaApiFn('GetUserAllInfo', {});
}
/**
...
export async function getUserInfoFromCloudSDK(force = false) {
    try {
        // 从cloud-sdk获取信息
        const wedaUser = await CLOUD_SDK.getUserInfo(force);
